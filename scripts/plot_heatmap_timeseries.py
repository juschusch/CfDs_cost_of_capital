...
if __name__ == "__main__":
    ...
    # Create a mapping from component name to carrier for relevant components
    component_carriers = pd.concat(
        [
    ...
    # Drop duplicates to handle components with the same name across different types
    component_carriers = component_carriers[~component_carriers.index.duplicated()]

    # filter for build capacities
    optimal_capacity = (
        n.statistics.optimal_capacity(nice_names=False).groupby(component_carriers).sum()
    )
    built_idx = optimal_capacity.where(optimal_capacity > 100).dropna().index

    # utilisation rates
    cf_series_or_df = n.statistics.capacity_factor(
        aggregate_time=False, nice_names=False
    )
    if isinstance(cf_series_or_df, pd.Series):
        cf_series_or_df = cf_series_or_df.to_frame()

    cf_df = cf_series_or_df.dropna(how="all", axis=1)
    
    # Group by carrier, aggregate, transpose, and scale
    cf = cf_df.groupby(component_carriers, axis=1).sum().T.mul(100)
