...
def add_co2_tracking(n, costs, options, sequestration_potential_file=None):
    ...
    if options["regional_co2_sequestration_potential"]["enable"]:
        ...
        if isinstance(e_nom_max, float):
            e_nom_max = pd.Series(e_nom_max, index=[spatial.co2.locations])
        #
...
def insert_electricity_distribution_grid(
    ...
    if len(fn) > 0:
        potential_df = pd.read_csv(fn, index_col=["bus", "bin"])
        potential = potential_df.squeeze()
        if not isinstance(potential, pd.Series):
            potential = pd.Series(potential, index=potential_df.index)
