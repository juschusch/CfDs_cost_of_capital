...
def resource_class_share_constraint(n, snapshots, snakemake):
    ...
    for tech in target_techs:
        ...
        # Import average capacity factors for the resource classes
        ...
        for bus, group in gens_by_bus:
            ...
            for i, gen_name in enumerate(group.index):
                # Constraint: p_nom_i = share_i * Total
                lhs = p_nom_opt_vars.loc[gen_name] - shares_series.loc[gen_name] * sum_p_nom_opt
                n.model.add_constraints(lhs == 0, name=f"ResourceClassShare_{bus}_{tech}_bin{i}")

                # Ensure that the model does not return the trivial solution of all zeros.
                n.model.add_constraints(p_nom_opt_vars.loc[gen_name] >= 1, name=f"ResourceClassShare_nontrivial_{bus}_{tech}_bin{i}")
